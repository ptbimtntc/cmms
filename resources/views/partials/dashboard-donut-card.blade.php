<div class="bg-white rounded-xl shadow-sm p-4 flex flex-col {{ $height ?? '' }}">
            ...
            <div class="w-full max-w-40 space-y-1">
                @foreach($legend as $item)
                    <div class="flex items-center justify-between text-xs">
                        ...
                        <div class="text-gray-800 font-medium shrink-0 pl-1">{{ $item['value'] }} <span class="text-gray-400 font-normal">({{ $item['percent'] }}%)</span></div>
                    </div>
                @endforeach
            </div>
        @endif
