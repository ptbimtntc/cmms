...
@section('content')
    <div class="mx-auto max-w-3xl" x-data="{ manualOpen: false }">
        ...
        {{--
            FreeDOMS offline-first (Task 7). Embedded, minimal machine list +
            condition options for resources/js/oil-audits/scan.js to seed the
            offline machine cache (Task 3's MasterDataCache) and build the
            offline entry panel below — never the whole machines table, only
            the fields OIL_AUDIT_CREATE needs. Purely additive: an online
            device never reads this data at all.
        --}}
        <div id="oil-audit-scan-data"
            data-machines='@json($offlineMachines ?? [])'
            data-conditions='@json($offlineConditions ?? [])'
            data-autostart-scanner="{{ session('success') ? 'true' : 'false' }}"></div>

        <div id="oil-audit-offline-feedback" class="mb-4 hidden rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800"></div>

        <div id="oil-audit-scanner-view">
        ...
        {{--
            Offline-only inline entry panel (Task 7 section 6/8) — hidden
            unless resources/js/oil-audits/scan.js determines the device is
            offline AND the scanned/typed machine is in the local cache.
            Built entirely client-side; the online scan -> entry page
            navigation above is completely untouched.
        --}}
        <div id="oil-audit-offline-panel" class="hidden rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
            <button type="button" id="oil-audit-offline-rescan" class="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-slate-900">
                <span aria-hidden="true">←</span> Scan mesin lain
            </button>
            <div id="oil-audit-offline-machine-info"></div>
            <div class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-5" id="oil-audit-offline-condition-buttons"></div>
        </div>
    </div>
